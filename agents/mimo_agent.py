"""
MIMO agent implementation for Multi-AI Debate Agent.
Implements the BaseAgent interface for MIMO model.
"""
...
class MIMOAgent(BaseAgent):
    """
    MIMO model agent implementation.

    Uses LangChain for unified interface with MIMO API.
    """

    def __init__(self):
        super().__init__(
            model_name=config.MIMO_MODEL,
    ...
    async def _get_embedding_impl(self, text: str) -> List[float]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code != 200:
                raise Exception(f"MIMO embedding API error: {response.status_code}")
