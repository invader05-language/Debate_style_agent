...
from backend.schemas.memory import MemoryCreate, MemoryResponse, MemorySearchResponse, MemoryUpdate
...
@router.get(
    ...
    response_model=MemorySearchResponse,
    summary="搜索记忆",
    description="支持语义搜索（pgvector）和文本搜索（ILIKE）的记忆搜索",
    responses={
        422: {"description": "搜索关键词为空"}
    }
)
...
async def create_memory(
    ...
    memory = MemoryModel(
        topic=body.topic,
        debate_summary=body.debate_summary,
...
async def update_memory(
    ...
    await db.commit()
    await db.refresh(memory)
    await cache.delete(f"memory:{memory_id}")
