...
import {
  Search, HelpCircle, BookOpen, MessageSquare, Github, Download,
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        // fallback to empty — will show static content
      } finally {
        setLoading(false);
      }
    }
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm space-y-4">
        ...
        <div className="divide-y divide-gray-100">
          {filteredFaqs.length === 0 && !loading && (
            <p className="py-6 text-center text-sm text-gray-400">No FAQs found. Check back later.</p>
          )}
          {filteredFaqs.map((faq, index) => {
            const isExpanded = expandedIndex === index;
            return (
              <div key={faq.id || index} className="py-4 first:pt-0 last:pb-0 font-medium">
                <button
                ...
                {isExpanded && (
                  <p className="mt-2.5 text-xs text-gray-550 leading-relaxed pl-1">{faq.answer}</p>
                )}
              </div>
            );
          })}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm space-y-4 flex flex-col justify-between">
          ...
          <div className="space-y-3 pt-2">
            <a href="https://discord.gg/invite" target="_blank" rel="noopener noreferrer"
        ...
        <div className="bg-purple-50/50 border border-purple-100 rounded-2xl p-6 shadow-sm flex flex-col justify-between">
          <div>
            <div className="flex justify-between items-start mb-3">
              <span className="text-3xs font-bold text-purple-700 bg-purple-100 px-2.5 py-0.5 rounded-full tracking-wider uppercase">Featured Resource</span>
          ...
          <button onClick={() => alert("Downloading the technical specifications whitepaper... (Simulated payload)")}
