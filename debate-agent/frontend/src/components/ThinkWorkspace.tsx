...
import {
  Brain, Terminal, Cpu, CheckSquare, Square, Flame, TrendingUp, Zap,
  Loader2, BookOpen, ShieldAlert, ThumbsUp, Award, ChevronRight
} from 'lucide-react';
import { NEUTRAL_ARBITERS } from '../data';
import { thinkApi } from '../api';
...
export default function ThinkWorkspace() {
  ...
  const [selectedThinkers, setSelectedThinkers] = useState<string[]>(["MIMO-4 Turbo", "Claude 3.5 Sonnet", "DeepSeek Coder"]);
  ...
  const { messages: wsMessages } = useWebSocket(currentTaskId || undefined);
  ...
  const handleToggleThinker = (name: string) => {
    if (selectedThinkers.includes(name)) {
      if (selectedThinkers.length > 1) setSelectedThinkers(selectedThinkers.filter(t => t !== name));
    } else {
  ...
  const handleTriggerEngine = async () => {
    ...
    for (let loop = 1; loop <= 3; loop++) {
      ...
      for (const thinker of selectedThinkers) {
        ...
        setTerminalStreams(prev => ({ ...prev, [thinker]: (prev[thinker] || "") + textUpdate }));
        await new Promise(resolve => setTimeout(resolve, 800));
      }
    }

    // Call real API for synthesis
    setActiveStep(`Running Dialectical Synthesis with ${consensusJudge}...`);
    try {
      // Create think session
      const session = await thinkApi.create({
        topic: problemDescription,
        depth: maxLoops > 5 ? "deep" : maxLoops > 2 ? "standard" : "quick",
        models: selectedThinkers,
      });
      setCurrentTaskId(session.id);

      // Start think session
      setActiveStep("Processing with AI models...");
      const result = await thinkApi.start(session.id);

      // Poll for completion
      let attempts = 0;
      let finalResult = result;
      while (finalResult.status === 'running' && attempts < 60) {
        await new Promise(resolve => setTimeout(resolve, 2000));
        finalResult = await thinkApi.get(session.id);
        attempts++;
      }

      if (finalResult.synthesis) {
        // Parse synthesis into report
        const lines = finalResult.synthesis.split('\n').filter(l => l.trim());
        const consensus: string[] = [];
        const divergences: string[] = [];
        let currentSection = 'consensus';

        for (const line of lines) {
          if (line.toLowerCase().includes('diverge') || line.toLowerCase().includes('conflict') || line.toLowerCase().includes('风险')) {
            currentSection = 'divergence';
          } else if (line.toLowerCase().includes('insight') || line.toLowerCase().includes('核心')) {
            currentSection = 'insight';
          }
          const cleaned = line.replace(/^[-*•]\s*/, '').replace(/^\d+[.、]\s*/, '').trim();
          if (cleaned.length > 10) {
            if (currentSection === 'consensus') consensus.push(cleaned);
            else if (currentSection === 'divergence') divergences.push(cleaned);
          }
        }

        setReport({
          consensusPoints: consensus.length > 0 ? consensus : finalResult.insights || [finalResult.synthesis.slice(0, 200)],
          conflictDivergences: divergences.length > 0 ? divergences : ["Multiple models provided complementary perspectives."],
          bestInsight: finalResult.insights?.[0] || finalResult.synthesis.slice(0, 300),
          bestInsightSource: `Sourced from ${selectedThinkers.join(' & ')}`,
          confidenceScore: 88,
        });
      } else {
        // Fallback
        setReport({
          consensusPoints: ["Implement lock-free Ring Buffers for handling high concurrent ingress.", "Partition message queues across distinct clusters to ensure fail-safety.", "Adopt a zero-allocation schema format like FlatBuffers to bypass GC pausing."],
          conflictDivergences: ["MIMO-4 favors persistent SSD storage, while Claude flags replication delay risks.", "Strict database replication consistency limits peak write performance."],
      ...
      // Update terminal streams with real results
      if (finalResult.messages) {
        for (const msg of finalResult.messages) {
          const thinkerName = selectedThinkers.find(t => t.toLowerCase().includes(msg.model_id?.toLowerCase() || '')) || selectedThinkers[0];
          setTerminalStreams(prev => ({
            ...prev,
            [thinkerName]: (prev[thinkerName] || "") + `\n\n> [FINAL] ${msg.content.slice(0, 200)}...\n> COMPLETE`
          }));
        }
      }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-4 bg-white border border-gray-200 rounded-2xl p-5 shadow-sm space-y-5">
          ...
          <div className="space-y-1.5">
            ...
            <textarea rows={4} value={problemDescription} onChange={(e) => setProblemDescription(e.target.value)}
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-2">
              {thinkersList.map((tk) => {
                ...
                return (
                  <button key={tk.name} onClick={() => handleToggleThinker(tk.name)}
                    className={`w-full flex items-start gap-3 p-3 rounded-xl border text-left transition-all ${isSelected ? 'bg-purple-50/50 border-purple-200' : 'bg-white border-gray-200 hover:bg-gray-50'}`}>
          ...
          <div className="space-y-1.5">
            ...
            <select value={consensusJudge} onChange={(e) => setConsensusJudge(e.target.value)}
              ...
              {NEUTRAL_ARBITERS.map((a, i) => (<option key={i} value={a}>{a}</option>))}
            </select>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <input type="range" min="1" max="10" value={maxLoops} onChange={(e) => setMaxLoops(parseInt(e.target.value))}
              className="w-full accent-purple-600 cursor-pointer h-1.5 bg-gray-100 rounded-lg" />
          </div>

          <div className="pt-4 border-t border-gray-100">
            <button onClick={handleTriggerEngine} disabled={isThinking}
          ...
          <div className="bg-[#1c1921] text-purple-100 border border-purple-950 rounded-2xl p-5 shadow-lg space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {selectedThinkers.map((thinker) => (
                <div key={thinker} className="bg-[#121016] border border-purple-950/60 rounded-xl p-4 flex flex-col justify-between h-[150px] shadow-inner">
          ...
          {report && (
            <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm space-y-6 animate-fade-in">
                ...
                <div className="space-y-3">
                  ...
                  <ul className="space-y-2">
                    {report.conflictDivergences.map((pt, i) => (
                      <li key={i} className="flex gap-2 text-xs text-gray-600 leading-relaxed font-medium">
                        <span className="text-red-500 font-bold">•</span>{pt}
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              <div className="bg-purple-50/50 border-l-4 border-purple-600 rounded-r-xl p-4 mt-4 space-y-1">
                ...
                <p className="text-sm italic font-medium text-purple-900 leading-relaxed">"{report.bestInsight}"</p>
