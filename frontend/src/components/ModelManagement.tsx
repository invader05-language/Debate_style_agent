...
export default function ModelManagement({ models, onToggleModel, onAddNewModel }: ModelManagementProps) {
  ...
  const handleTestConnection = (id: string) => {
    ...
    setTimeout(() => {
      const generatedLatency = Math.floor(Math.random() * 80) + 110;
      setTestResults(prev => ({
        ...prev,
        [id]: { status: "Online", latency: `${generatedLatency}ms` }
      }));
  ...
  return (
    <div className="space-y-6">
              ...
              {models.map((model) => {
                ...
                return (
                  <tr key={model.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4.5">
                      <button 
                        ...
                        className={`w-9 h-5 rounded-full p-0.5 transition-colors focus:outline-none cursor-pointer ${
                          model.toggle ? 'bg-green-500' : 'bg-gray-200'
                        }`}
                      >
                        <div className={`w-4 h-4 rounded-full bg-white shadow transition-transform ${
                          model.toggle ? 'translate-x-4' : 'translate-x-0'
                        }`} />
                      </button>
                    </td>
                    ...
                    {/* Provider */}
                    <td className="px-6 py-4.5 text-xs text-gray-600 font-medium">
                      {model.provider}
                    </td>

                    {/* Context size */}
                    <td className="px-6 py-4.5">
                      <span className="text-2xs font-semibold px-2 py-0.5 bg-gray-100 text-gray-600 rounded-md">
                        {model.contextWindow}
                      </span>
                    </td>

                    {/* API Schema */}
                    <td className="px-6 py-4.5 text-xs text-gray-500 font-mono">
                      {model.apiFormat}
                    </td>
      ...
      {/* Bottom Grid: Speed-Boost & Inference Analytics */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        
        {/* Speed Boost Promo panel */}
        <div className="lg:col-span-5 bg-gradient-to-br from-[#23005c] to-[#40128a] text-white rounded-2xl p-6 shadow-md flex flex-col justify-between">
          <div>
            <div className="flex justify-between items-start mb-4">
              <span className="text-3xs font-bold px-2 py-1 bg-white/20 text-indigo-100 rounded-full tracking-wider uppercase">
                Pro Plan Advantage
              </span>
        ...
        {/* Inference Distribution Visual chart */}
